...
def convert_to_sub2run_dict(raws: list) -> Dict[str, List[any]]:
    ...
    subject_runs_raw = defaultdict(list)  # Use defaultdict to store the subjects and their runs
    ...
    for raw_object in raws:
        ...
        if subject_match and run_match:
            subject_runs_raw[''.join(['sub-', subject_match.group(1)])].append(raw_object)
...
def load_raw_bids(bids_root: str) -> Dict[str, List[any]]:
    ...
    bids_paths = mne_bids.find_matching_paths(bids_root,
                                              datatypes='ieeg',
                                              sessions=mne_bids.get_entity_vals(bids_root, entity_key="session",
                                                                                ignore_sessions="on"),
                                              extensions='.vhdr')
    return convert_to_sub2run_dict([mne_bids.read_raw_bids(bids_path=bids_path, verbose=False)
...
def load_info(path: str) -> pd.DataFrame:
    ...
    if path.lower().endswith('.csv'):
        ...
        raise ValueError("Unsupported file format. Supported formats are .csv and .mat.")
...
def load_markers(marker_path: str, data_path: str) -> Dict[str, Dict[str, List[float]]]:
    ...
    if data_set == 'Gestures':  # TODO move to csv file
        data_num = 1
        motor_strategy = {'0': 'gesture D', '1': 'gesture F', '2': 'gesture V',
                          '3': 'gesture Y'}  # same names as in BIDS, otherwise broadcasting error
        sub_code = {'mels': 'sub-01_run-01', 'terp_run1': 'sub-02_run-01', 'terp_run2': 'sub-02_run-02',
                    'brem': 'sub-03_run-01', 'rooi': 'sub-04_run-01', 'habe': 'sub-05_run-01'}
    elif data_set == 'BoldFingers':
        data_num = 7
        motor_strategy = {'0': 'little move', '1': 'index move',
                          '2': 'thumb move'}  # same names as in BIDS, otherwise broadcasting error
        sub_code = {'rooi': 'sub-01_run-01', 'heek': 'sub-02_run-01', 'franeker': 'sub-03_run-01',
                    'ommen_run1': 'sub-04_run-01', 'ommen_run2': 'sub-04_run-02',
                    'vledder': 'sub-05_run-01', 'mels': 'sub-06_run-01', 'duiven': 'sub-07_run-01',
                    'habe': 'sub-08_run-01'}
    else:
        ValueError(f'Accepts only onset markers for "Gesture" or "BoldFingers" data set. Got {data_set}')
    ...
    if marker == 'gammaSlopemarkers':
        ...
        ValueError(f'Accepts only a path to the marker with a file name "movementOnsetMarkers" '
    ...
    def load_mat_file(file_path: str, motor_strategy: Dict[str, str], sub_code: Dict[str, str], marker_id: str) -> \
        ...
        assert len(sub) == 1, RuntimeWarning(f'No code or multiple codes matches the file: {file_path}')

        # Load .mat file
        raws = scipy.io.loadmat(file_path)
        assert len(motor_strategy) == raws[marker_id].shape[1], (f'The motor strategy and .mat file for subject {sub} '
    ...
    mat_files = [os.path.join(marker_path, filename) for filename in os.listdir(marker_path) if
                 filename.startswith(f'{marker[:-1]}_{data_num}') and
                 (re.match('^((movementOnsetMarker)|(gammaSlopemarker))_\d+_[a-zA-Z]+(?:_run\d+)?\.mat$', filename) or
                  re.match('^((movementOnsetMarker)|(gammaSlopemarker))_\d+_[a-zA-Z]+\.mat$', filename))]
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        results = list(executor.map(load_mat_file, mat_files,
