...
from optimizer.backend._backend import channel_id_to_int
...
def elimination_plot(
        ...
        output_path: str,
        identifier: str = "",
        ied_ed: Optional[Tuple[float, float]] = None,
    ...
    result_grid["Mean"] *= 100  # Scale score for readability
    ...
    if ied_ed is not None:
        if all(
                [
                    method in ["SpatialExhaustiveSearch",
            ...
            UserWarning(
                f"Computation of the Area (using ied_ed = {ied_ed}) is only possible for SES and SSHC"
            )
    ...
    grid_plot = plt.GridSpec(
        2,
        2,
        hspace=0.2 / height,
        wspace=0.2 / width,
        height_ratios=(1, 8),
        width_ratios=(4 * (grid.size / 32), 1),
    )
    ...
    ax_main.set_xlim(0, (
        xlim_max + 1 if target == "Size" else xlim_max + xlim_min))
    ax_main.set_ylim(0, 101.5)
    xtick = int(xlim_max / (8 * (grid.size / 32)))
    ax_main.set_xticks(
        np.arange(0, xlim_max + (xtick if target == "Size" else 0), xtick)
    )
    ...
    ax_main.grid(which="major", linewidth=0.3, linestyle="--", color="grey",
                 alpha=0.25)
...
def importance_plot(
    ...
    result_grid = copy(result_grid)
    new_column = result_grid["Mask"].apply(
        lambda x: grid[x.reshape(grid.shape)])
    ...
    top_grids = result_grid.nlargest(top_k, "Mean")
    ...
    for row in grid:
        for channel_id in row:
            ...
            channel_rows = result_grid[
                result_grid["Channel IDs"].apply(
                    lambda ids: channel_id in [int(i) for i in ids]
                )
            ]

            weighted_score = np.average(
                channel_rows["Mean"]
            )  # , weights=1 / channel_rows['Grid Size'])
    ...
    numerical_data = np.array(
        [
            [
                (
                    channel_scores.get(channel_id, np.nan)
                    if channel_id not in bad_channels
                    else np.nan
                )
                for channel_id in row
            ]
    ...
    for row in grid:
        for channel_id in row:
            ...
            scores = channel_rows["Mean"].values
    ...
    custom_greens = LinearSegmentedColormap.from_list("custom_greens",
                                                      colors[64:])
    ...
    main_ax.set_title(f"Channel Importance for {identifier}",
                      fontsize=font_size)
    main_ax.set_xticks([])
    main_ax.set_yticks([])
    ...
    if viz == "grid_overlay" and not spatial:
        warnings.warn(
            f"Grid overlay visualization only allowed for spatial algorithms",
            UserWarning,
        )
        return None
    ...
    for row_idx, row in enumerate(grid):
        for col_idx, channel_id in enumerate(row):
            ax = plt.subplot2grid(
                (grid.shape[0], grid.shape[1]), (row_idx, col_idx), fig=fig
            )
            if channel_id in bad_channels:
                ax.add_patch(
                    Rectangle((0, 0), 1, 1, fill=True, facecolor="white",
                              linewidth=0)
                )
                ...
                if (
                        channel_id
                        in grid[
                    result_grid["Mask"].ravel() == max(result_grid["Mean"])]
                ):
                    ax.add_patch(
                        Rectangle(
                            (0, 0),
                ...
                ax.text(
                    ...
                    transform=ax.transAxes,
                    fontsize=7 - grid.size * 0.02,
                    fontweight="bold",
                    color="white",
                )
            else:
                raise ValueError(
                    f"Visualization method is not supported. Got {viz}")
    ...
    if viz == "grid_overlay" and spatial:
        ...
        for channels, color, k in zip(top_channels_list, top_k_colors,
                                      range(top_k)):
    ...
    plt.subplots_adjust(wspace=0,
                        hspace=0)  # Make cells adjacent to each other
    plt.savefig(
